...
def create_model():
    ...
    model.add(LSTM(hidden_neurons,
                   batch_input_shape=(None, length_of_sequences, 3)))
...
X, Y = create_train_data(high, low, end, adj_end, up_down_rate, ommyou, length_of_sequences)


# train_x, test_x, train_y, test_y  = cv.train_test_split(X, Y, test_size=0.2)
...
history = model.fit(train_x, train_y, batch_size=10, epochs=epochs, verbose=1, validation_split=0.2)
