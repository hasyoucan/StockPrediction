...
def create_train_data(adj_starts, high, low, adj_ends, ommyo_rate, y_data, samples):
    udr_start = pct_change(adj_starts)
    udr_high = pct_change(high)
    udr_low = pct_change(low)
    udr_end = pct_change(adj_ends)

    # 銘柄×日付→日付×銘柄に変換
    # transposed = udr_start.values
    # transposed = udr_end.values
...
def test_predict(stock_data_files, target_stock):
    adj_starts, high, low, adj_ends, ommyo_rate = load_data(stock_data_files)

    _y_data = pct_change(adj_starts[target_stock])
    # _y_data = pct_change(adj_ends[target_stock])
    # _y_data = ommyo_rate[target_stock]
    y_data = pd.cut(_y_data, category_threshold, labels=False).values
...
if __name__ == '__main__':
    ...
    test_predict(stock_data_files, target_stock)
